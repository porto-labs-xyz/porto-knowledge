...
Use versioned logical schema/ruleset upgrades and offline checked migration with a retained pre-upgrade checkpoint. Pin compatible RocksDB/binding versions. No old owner may write after promotion of a new schema. Rebuild indexes from authoritative state and verify the portable logical digest; never equate an SST-file hash with logical state identity. Rollback must not delete posted entries, rewind chain history or retry unknown transfers. Refunds and corrections are new events. Maintain a named support contact for listener access, artist statement questions and operator onboarding; one existing support channel is enough.
